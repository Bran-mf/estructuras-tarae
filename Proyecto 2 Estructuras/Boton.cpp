...
Boton::Boton(string accion, string texto, float x, float y, string url, float tx, float ty)
{
	this->x = 0; this->y = 0;
...
Boton::Boton(string accion, string texto, float x, float y, string url, float tx, float ty, float scaleX, float scaleY)
{
...
Boton::Boton(string accion, string texto, float x, float y, string url, float tx, float ty, float scaleX, float scaleY, float orX, float orY)
{
	this->x = 0; this->y = 0; this->alto = 0; this->ancho = 0; this->tx = 0; this->ty = 0;
	this->accion = accion;
	this->texto = texto;
...
Boton::Boton(string accion, float x, float y, string url, float scaleX,float scaleY)
{
	this->accion = accion;
	texture = new Texture();
...
Boton::Boton(string accion, float x, float y, string url, float scaleX, float scaleY, float orX, float orY)
{
	this->accion = accion;
	texture = new Texture();
	...
	if (texture->loadFromFile(url)) {
		...
		if (orX != 0 && orY != 0) {
			...
			orX = sprite->getPosition().x + ancho / 2;
			orY = sprite->getPosition().y + alto / 2;
			x = x + (800 / 19) / 2;
			y = y + (720 / 21) / 2;
...
void Boton::setAncho(float ancho)
{
	this->ancho = ancho;
	if (sprite != NULL) {
		sprite->setScale(alto, ancho);
